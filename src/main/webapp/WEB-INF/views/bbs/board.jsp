	...
	<div class="wrapper row-offcanvas row-offcanvas-left">
		...
		<div class="table">
			...
			<div align="right" style="margin-bottom: 2px;">
				<form action="board" method="GET" name="searchForm">
			...
			<!-- 게시판 리스트 출력 -->
				...
				<ul>
					<li class="no">${board.getBbsNo()}</li>
					<li class="subject"><a href='view?bbsno=${board.getBbsNo()}'>${board.getTitle()}</a>
						<c:if test="${board.getIsfile() == 'T' }">&nbsp;&nbsp;<span
								class="glyphicon glyphicon-paperclip"></span>
						</c:if> <c:if test="${board.getNewdata() < 3 }">&nbsp;&nbsp;<span
								class="label label-warning">New</span>
						</c:if></li>
					<li class="name">${board.getWriter()}</li>
					<li class="date">${board.getRegdate()}</li>
					<li class='hits'>${board.getViewcnt()}</li>
					</ul>
				</c:forEach>
			...
			<div class="input-group-btn">
				<button id="add-new-event" type="button"
					class="btn btn-default btn-flat" onclick="window.location='write'">Write</button>
			</div>
		</div>
